...
> 最后更新：2026-09-16
> 当前分支：`main`；显示/打包版本 **`1.7.6-debug-1`**。基线 `2002f6b` 已与 `origin/main` 同步；本轮作业状态、校历导出、iOS 校历跳转、会话恢复、课件刷新/下载和物理在线底栏改动待提交、未打 tag、未推送。Windows 本机无法打 IPA，由用户在 Mac 打包。
...
- **2026-09-16 首页截止与物理在线安排**：首页周日期格有作业/物理在线截止时使用红色容器，详情行的“截止/物理截止”标签使用错误色；物理在线页面将“课程作业”置于“作业时间安排”之前，安排按当前选中课程过滤，首页总议程仍保留全部课程。CDP 已核对 Moodle 日历事件的 `mod_assign`/`due` 属性、作业链接和课程归属；Android 最终包视觉核对通过，两门课不再在物理在线页混排。
...
- **2026-09-16 物理在线底栏入口**：新增持久化偏好“显示物理在线入口”，默认开启；紧凑端底栏顺序为“作业 → 物理在线 → 更多”，关闭后物理在线仍保留在“更多”中。底栏与“更多”复用同一 `PHYVLAB` 路由/原生页面；Android 模拟器已验证六项底栏、点击进入 `NativeDetailActivity`、设置开关隐藏/恢复，最终恢复为开启。
...
1. **验收 `1.7.6-debug-1`**：先使用本轮普通 CI artifacts 做 Android/Windows/macOS/iOS 验收；本轮不打 tag、不建 GitHub Release。
