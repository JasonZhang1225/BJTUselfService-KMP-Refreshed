...
> 最后更新：2026-09-16
> 当前分支：`main`；显示/打包版本 **`1.7.6-debug-1`**。主体提交 `eca1a3a`、`25947a9`、`591c905`、`e7f8efb` 已推送；打包已暂停，未打 tag、未建 Release。Windows 本机无法打 IPA，由用户在 Mac 打包。
...
- **2026-09-16 物理在线总开关**：移除“更多”中的物理在线页面入口，改为总开关；开启同时启用自动同步和“作业 → 物理在线 → 更多”底栏入口，关闭则不自动同步、不显示首页物理日程和底栏入口；开关固定放在“更多”的物理在线行，设置页的两处旧开关已移除。Android 模拟器已验证关闭/开启后底栏分别为 5/6 项，开关状态持久化；物理在线页仍为一级页面，作业详情才进入 `PHYVLAB_DETAIL`。
- **2026-09-16 课表模式文案**：课表切换按钮改为“色块-概览”和“课程名-列表”，明确两种视图的内容差异。
