...
> 最后更新：2026-09-16
> 当前分支：`main`；显示/打包版本 **`1.7.6-debug-1`**。主体提交 `eca1a3a` 已推送；本轮物理在线底栏一级页与移除页面“安排”区块的修订待提交、未打 tag、未建 Release。Windows 本机无法打 IPA，由用户在 Mac 打包。
...
- **2026-09-16 物理在线底栏入口**：新增持久化偏好“显示物理在线入口”，默认开启；紧凑端底栏顺序为“作业 → 物理在线 → 更多”，关闭后仍可从“更多”进入一级物理在线页。底栏和“更多”复用同一 `PHYVLAB` 一级路由，作业详情才进入 `PHYVLAB_DETAIL`；Android 模拟器已验证六项底栏、点击后仍在 `MainActivity` 且页面仅有课程作业，最终恢复为开启。
