...
> 最后更新：2026-09-17
> 当前分支：`main`；显示/打包版本 **`1.7.6-KMP`**。正式提交 `ac10ba1`、CI 修复 `eb42f1e` 已推送；正式 tag `v1.7.6-KMP` 已指向 `eb42f1e`。GitHub Actions run `35185935353` 的 Android、Windows、macOS、iOS 和 GitHub Release 五个 job 全部成功，正式 Release 已发布。Windows 本机无法打 IPA，由用户在 Mac 打包。
> 阶段状态：**1.7.6-KMP 已正式发布；首页周议程高度/加载态修复、校历周定位、物理在线、作业会话恢复、课件刷新和 Windows CI 打包收口均已合入。** 邮箱主体能力保持自 `v1.7.5-KMP`，真实邮箱写操作、Apple 真机签名等仍按已知限制保留。
...
- **2026-09-17 正式打包与发布**：远端首次 run 暴露 Android action 请求 legacy `tools` 包、Windows runner 中文包名触发 WiX 311；已分别限制 Android action 只装 `platform-tools`，并为 CI 恢复 ASCII 安装器元数据、本机保留中文默认名。修复后的 run `35185935353` 五个 job 全部成功，`v1.7.6-KMP` 正式 Release 已上传 arm64 APK、Windows MSI、macOS DMG 和 iOS unsigned IPA。
- **2026-09-16 Windows 侧栏与安装器中文名**：宽屏侧栏现在复用物理在线总开关，开启时显示物理在线一级入口，关闭时隐藏且保留“更多”中的开关入口。Windows 本机默认 `packageName`、描述和菜单组仍为中文；CI 使用 ASCII 安装器元数据避免 WiX 311。正式远端 MSI 已在 `35185935353` 验证成功。
...
1. **正式包安装回归**：从 `v1.7.6-KMP` Release 下载 Android arm64、Windows MSI、macOS DMG 和 iOS unsigned IPA，分别做覆盖安装/侧载检查；Windows 卸载清凭据仍待专项复测。
