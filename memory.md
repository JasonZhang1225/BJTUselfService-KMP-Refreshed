...
> 最后更新：2026-09-16
> 当前分支：`main`；显示/打包版本 **`1.7.6-debug-1`**。主体提交 `eca1a3a`、`25947a9`、`591c905`、`d5b1029`、`40e7f9b`、`fa1e1d6`、`d8a89e9` 已推送；本轮首页周卡片 pager 高度隔离待提交。Windows 本地 MSI 已成功生成，远端普通打包未触发，未打 tag、未建 Release。Windows 本机无法打 IPA，由用户在 Mac 打包。
...
- **2026-09-16 首页周卡片高度隔离**：移动端首页周卡片的 `HorizontalPager` 记录每个教学周页面的实际内容高度，切换周时使用当前页高度；周 2 的长日程不会再把周 3 撑成同样高度。目标页未测量时暂时自然测量，避免切换到远处周页时被旧高度裁剪。首页测试、Windows 编译和 Android 编译通过；iOS 仍待 Mac 验证。
